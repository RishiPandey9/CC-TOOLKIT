/**
 * Comprehensive Automated Verification Test Suite for CC-Toolkit
 * Tests all ciphers, S-DES, cryptanalysis heuristics, multi-layer pipelines, and DSA math engines.
 */

import { caesarEncrypt, caesarDecrypt, crackCaesarChiSquared } from '../src/lib/crypto/caesar';
import { monoEncrypt, monoDecrypt, generateMonoKey, isValidMonoKey } from '../src/lib/crypto/monoalphabetic';
import { playfairEncrypt, playfairDecrypt, getPlayfairMatrix } from '../src/lib/crypto/playfair';
import { vigenereEncrypt, vigenereDecrypt, calculateIoC, estimateVigenereKeyLength } from '../src/lib/crypto/vigenere';
import { railfenceEncrypt, railfenceDecrypt } from '../src/lib/crypto/railfence';
import { rowEncrypt, rowDecrypt } from '../src/lib/crypto/rowcolumn';
import { hillEncrypt, hillDecrypt, parseHillMatrix, SAMPLE_HILL_KEYS } from '../src/lib/crypto/hill';
import { affineEncrypt, affineDecrypt, generateRandomAffineKeys } from '../src/lib/crypto/affine';
import { rot13, atbash } from '../src/lib/crypto/atbash';
import { sdesEncryptBlock, sdesDecryptBlock, generateSDesKeys, sdesEncryptText, sdesDecryptText } from '../src/lib/crypto/sdes';
import { executeMultiLayerPipeline, CipherLayer } from '../src/lib/crypto/multilayer';
import { analyzeFrequencies, evaluateDecryptionFitness, crackMonoWithHillClimbing, crackKnownPlaintext } from '../src/lib/crypto/monocracker';
import { extendedGcd, modInverse, isCoprime } from '../src/lib/dsa/extended-gcd';
import { det2x2, det3x3, inverse2x2, inverse3x3 } from '../src/lib/dsa/matrix-math';
import { Trie } from '../src/lib/dsa/trie';
...
export function runAllAutomatedTests(): { results: TestCaseResult[]; total: number; passed: number; failed: number } {
  ...
  test('DSA Math', '2x2 and 3x3 Modular Matrix Inversion', () => {
    ...
    if (inv2) {
      // (mat * inv) mod 26 should equal Identity matrix [[1,0],[0,1]]
  ...
  test('Classical Ciphers', 'Playfair Cipher 5x5 Matrix & Digraph Processing', () => {
    ...
    assertEqual(matrix[1].join(''), 'CHYB D'.replace(' ', ''), 'Second row');
  ...
  test('Classical Ciphers', 'ROT13 & Atbash Symmetric Involutions', () => {
    ...
    assertEqual(rot13(rot13(text)), text, 'ROT13 is its own inverse: rot13(rot13(x)) == x');
    assertEqual(atbash(atbash(text)), text, 'Atbash is its own inverse: atbash(atbash(x)) == x');
  });

  // --- Suite 3: S-DES (Simplified DES) ---
  test('S-DES Engine', 'Key Schedule Subkeys K1 and K2 generation', () => {
    // Academic standard test vector: Key = 1010000010
  ...
  test('S-DES Engine', '8-bit Block Encrypt and Decrypt Known Academic Vector', () => {
    ...
    // Verified standard S-DES output
